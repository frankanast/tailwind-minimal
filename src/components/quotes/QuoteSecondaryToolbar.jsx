...
import {
    EyeIcon, LockClosedIcon, PencilIcon, PercentBadgeIcon, XMarkIcon,
...
export default function QuoteSecondaryToolbar() {
    ...
    const handleFormulaSubmit = () => {
        applyFormula() // Applies a math.js formula to the rates in all selected cards
        setFormulaDialogIsOpen(false);
    }

    const handleInputChange = (event) => {
        setFormulaInput(event.target.value); // Save input to context
    };

    const handleOpenDiscountDialog = () => {
        setDiscountDialogIsOpen(true);
    };

    const handleDiscountSubmit = () => {
        applyDiscount() // Applies a math.js formula to the rates in all selected cards
        setDiscountDialogIsOpen(false);
    }
    ...
    const actions = [
        ...
        { name: 'Discount', href: '#', icon: <PercentBadgeIcon />, handler: handleOpenDiscountDialog },
    ...
    return (
        <>
            ...
            <Transition.Root show={discountDialogIsOpen} as={Fragment}>
                <Dialog as="div" className="relative z-10" onClose={() => setDiscountDialogIsOpen(false)}>
                    <Transition.Child
                        as={Fragment}
                        enter="ease-out duration-300"
                        enterFrom="opacity-0"
                                ...
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                                    <div className="absolute right-0 top-0 hidden pr-4 pt-4 sm:block">
                                        <button
                                            type="button"
                                            className="rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none"
                                            onClick={() => setDiscountDialogIsOpen(false)}
                                        >
                                            <span className="sr-only">Close</span>
                                            <XMarkIcon className="h-6 w-6" aria-hidden="true" />
                                        </button>

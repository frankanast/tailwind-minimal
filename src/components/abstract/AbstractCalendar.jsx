...
export default function AbstractCalendar({ clickHandler, checkInDate, checkOutDate }) {
    const [currentMonth, setCurrentMonth] = useState(new Date());
    const [selectedRange, setSelectedRange] = useState([null, null]);

    useEffect(() => {
        // Ensure we are only setting valid dates in the selected range
        if (checkInDate || checkOutDate) {
            setSelectedRange([checkInDate || null, checkOutDate || null]);
        }
    }, [checkInDate, checkOutDate]);

    useEffect(() => {
        // Only update currentMonth if the dates are valid
    ...
    const renderCalendarDays = (month) => {
        ...
        return days.map((day, dayIdx) => {
            ...
            const isStartDay = selectedRange[0] instanceof Date && day && isSameMonth(day, month) && day.getTime() === selectedRange[0].getTime();
            const isEndDay = selectedRange[1] instanceof Date && day && isSameMonth(day, month) && day.getTime() === selectedRange[1].getTime();
            const isSelected = selectedRange[0] instanceof Date && selectedRange[1] instanceof Date && day && isWithinInterval(day, { start: selectedRange[0], end: selectedRange[1] });
    ...
    return (
        <div className="relative grid grid-cols-1 gap-x-6 md:grid-cols-2 w-full">
            <button
                type="button"
